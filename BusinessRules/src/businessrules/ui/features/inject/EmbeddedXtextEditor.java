...
public class EmbeddedXtextEditor {
	...
	private void createSourceViewer(Composite parent) {
		...
		fSourceViewer = fSourceViewerFactory.createSourceViewer(parent, fVerticalRuler, getOverviewRuler(), true,
				fStyle);
			...
			public void processIssues(List<Issue> issues, IProgressMonitor monitor) {
				if (annotationIssueProcessor == null) {
					annotationIssueProcessor = new AnnotationIssueProcessor(fDocument,
							fSourceViewer.getAnnotationModel(), new IssueResolutionProvider.NullImpl());
				}
				if (annotationIssueProcessor != null)
	...
	private void configureSourceViewerDecorationSupport(SourceViewerDecorationSupport support) {
		...
		Iterator<AnnotationPreference> e = Iterators.filter(fAnnotationPreferences.getAnnotationPreferences()
	 ...
	 *            fResource.setParentResource(eObject.eResource()); if (eObject
	 *            != null) { EObject asStringEObject = null; // XtextResource
	 *            asStringResource = (XtextResource)
	 *            fResourceSetProvider.get(null
	 *            ).createResource(URI.createURI("asStringResource." +
	 *            fFileExtension)); ResourceSet resourceSet =
	 *            fResourceSetProvider.get(null); EmbeddedXtextResource
	 *            asStringResource = fEmbeddedXtextResourceProvider.get();
	 ...
	 *            fFileExtension)); //$NON-NLS-1$
	 *            asStringResource.setParentResource(eObject.eResource()); try {
	 *            asStringResource.load(new StringInputStream(asString),
	 *            Collections.emptyMap()); if
	 *            (!asStringResource.getContents().isEmpty()) { asStringEObject
	 *            = asStringResource.getContents().get(0); } } catch
	 *            (IOException e) { // ignore, will set the string to the
	 *            serialization of the given eObject }
	 * 
	 *            try { EcoreUtil.resolveAll(asStringResource); } catch
	 *            (Exception e) { // ignore }
	 * 
	 *            if (!asStringResource.getErrors().isEmpty() ||
	 *            (asStringResource.getParseResult() != null &&
	 *            asStringResource.
	 *            getParseResult().getSyntaxErrors().iterator().hasNext())) { //
	 *            if there are parsing errors in the saved string, then we
	 *            update with it update(asString); } else if (asStringEObject !=
	 *            null) { try { // Resource copyResource = (XtextResource)
	 *            fResourceSetProvider
	 *            .get(null).createResource(URI.createURI("copyResource." +
	 *            fFileExtension)); // EmbeddedXtextResource copyResource =
	 *            (EmbeddedXtextResource)
	 *            fResourceSetProvider.get(null).createResource
	 *            (URI.createURI("copyResource." + fFileExtension));
	 *            EmbeddedXtextResource copyResource =
	 *            fEmbeddedXtextResourceProvider.get(); // EmbeddedXtextResource
	 *            copyResource = new EmbeddedXtextResource();
	 *            copyResource.setURI(URI.createURI("copyResource." +
	 *            fFileExtension)); //$NON-NLS-1$
	 *            copyResource.setParentResource(eObject.eResource()); try {
	 *            EObject copyEObject = EcoreUtil.copy(eObject);
	 *            copyResource.getContents().add(copyEObject);
	 *            EcoreUtil.resolveAll(copyResource); if (!equals(copyEObject,
	 *            asStringEObject)) { // String model =
	 *            getResource().getSerializer().serialize(copyEObject,
	 *            SaveOptions
	 *            .newBuilder().noValidation().format().getOptions());
	 *            update(asString); // FIXME: should update with the serialized
	 *            form of the copyEObject but throw RuntimeException!!! } else {
	 *            // if there is no error and the content are equals, then we
	 *            also update with the string update(asString); } } catch
	 *            (Exception e) { update(asString); }
	 * 
	 *            copyResource.unload(); } catch (Exception e) {
	 *            update(asString); } } else { update(""); //$NON-NLS-1$ }
	 * 
	 *            asStringResource.unload();
	 *            asStringResource.getResourceSet().getResources
	 *            ().remove(asStringResource); // } else { // update("");
	 *            //$NON-NLS-1$ } }
	 */
	...
	private final class SourceViewerFocusListener implements FocusListener {
		...
		public void focusGained(FocusEvent e) {
			...
			for (ActionHandler actionHandler : fActionHandlers) {
				fHandlerActivations.add(handlerService.activateHandler(actionHandler.getAction().getId(),
	 ...
	 * MatchModel match = null; try { match = MatchService.doMatch(expected,
	 * actual, options); DiffModel diff = DiffService.doDiff(match, false);
	 * return diff.getDifferences().isEmpty(); } catch (InterruptedException e)
